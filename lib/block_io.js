...
BlockIo.prototype._withdraw = function (path, args, cb) {
  ...
  var self = this;
  return this._request(path, args, function (e, res) {
    ...
    self._request('sign_and_finalize_withdrawal', {
...
BlockIo.prototype._sweep = function (path, args, cb) {
  ...
  var self = this;
  this._request(path, args, function (e, res) {
    if (e) return cb(e, res);
    ...
    self._request('sign_and_finalize_sweep', {
      signature_data: JSON.stringify(res.data)
    }, cb);
...
BlockIo.prototype._request = function (path, args, cb) {
  ...
  if (!cb) cb = function () {};
  const next = BlockIo.parseResponse(cb);

  this.client.request(method, url, args)
    .then(d => next(null, d.res, d.body))
    .catch(e => next(e));
};
